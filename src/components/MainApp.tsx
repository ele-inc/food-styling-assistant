...
import ImageConfirmation, {
	type RevisionRequest,
} from "@/components/ImageConfirmation";
import LayoutPreview from "@/components/LayoutPreview";
import MessageBubble from "@/components/MessageBubble";
import ModeSelector, { type WorkMode } from "@/components/ModeSelector";
...
function convertSessionToDbSession(
	session: Session,
	userId: string,
): DbSession {
	return {
...
export function MainApp({ user }: MainAppProps) {
	...
	const [pendingImagePrompt, setPendingImagePrompt] = useState<string | null>(
		null,
	);
	...
	const [showProposalConfirmation, setShowProposalConfirmation] =
		useState(false);
	const [awaitingModificationInput, setAwaitingModificationInput] =
		useState(false);
	...
	const [selectedDishes, setSelectedDishes] = useState<
		Array<{ order: number; id: string; name: string }>
	>([]);
	const [currentDishIndex, setCurrentDishIndex] = useState<number>(0);
	const [dishImages, setDishImages] = useState<
		Array<{ order: number; name: string; imageUrl: string }>
	>([]);
	...
	const handleNewSession = useCallback(
		async (productName?: string) => {
			...
			const title =
				safeProductName || `${modeLabel} ${now.toLocaleDateString("ja-JP")}`;
		...
		async (title: string) => {
			...
			try {
				await saveSessionToDb(
					convertSessionToDbSession(updatedSession, user.id),
				);
			} catch (error) {
	...
	const parseResponse = (response: string) => {
		...
		// コープレター：料理選択リストの解析
		const dishSelectionMatch = response.match(
		...
		// コープレター：3品選択確定の解析
		const dishesConfirmedMatch = response.match(
		...
		const layoutMatch = response.match(
		...
		if (proposalMatch) {
			try {
				...
				if (parsed.proposals && Array.isArray(parsed.proposals)) {
					const formattedProposals: Proposal[] = parsed.proposals.map(
						(p: {
							id: string;
							title: string;
							description: string;
					...
					if (
						parsed.theme &&
						currentSession &&
						currentSession.title.includes("コープレター")
					) {
						updateSessionTitle(parsed.theme);
					}
	...
	const handleProposalGenerateRequest = async () => {
		...
		await handleSendMessage(
			`案${selectedProposal.id}（${selectedProposal.title}）でお願いします。このままで大丈夫です。`,
		);
	};
	...
	const handleProceedToNext = async () => {
		...
		if (currentSession && user.id) {
			...
			try {
				await saveSessionToDb(
					convertSessionToDbSession(completedSession, user.id),
				);
				setSessions((prev) =>
					prev.map((s) =>
				...
				{awaitingModificationInput && selectedProposal && (
					<div className="bg-amber-50 border-t border-amber-200 px-4 py-2">
						<p className="text-sm text-amber-700">
							<strong>案{selectedProposal.id}</strong>
							の修正箇所を入力してください（例：お皿を白にしたい、盛り付けをもっと立体的になど）
						</p>
					</div>
				)}
