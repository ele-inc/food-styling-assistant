...
import { Package, Copy, Check, ShoppingCart, ChevronRight } from 'lucide-react';
...
const CATEGORY_LABELS: Record<EquipmentItem['category'], { label: string; color: string }> = {
...
export default function EquipmentList({ items, onProceedToNext }: EquipmentListProps) {
  const [copied, setCopied] = useState(false);
  const [checkedItems, setCheckedItems] = useState<Set<number>>(new Set());

  const handleCopyList = () => {
    const text = items
      .map(item => `□ ${item.name}：${item.quantity}${item.description ? `（${item.description}）` : ''}`)
      .join('\n');
        ...
        {Object.entries(groupedItems).map(([category, categoryItems]) => (
          <div key={category}>
            <div className="flex items-center gap-2 mb-3">
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${CATEGORY_LABELS[category as EquipmentItem['category']]?.color}`}>
                {CATEGORY_LABELS[category as EquipmentItem['category']]?.label}
              </span>
              <span className="text-xs text-gray-400">{categoryItems.length}点</span>
            </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline gap-2">
                      <span className={`font-medium ${checkedItems.has(item.originalIndex) ? 'text-gray-500 line-through' : 'text-gray-800'}`}>
                        {item.name}
                      </span>
