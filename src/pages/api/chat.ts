...
import { createGeminiClient, generateChatResponse, generateImage } from '@/lib/gemini';
    ...
    for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt += 1) {
      try {
        if (generateImagePrompt) {
          ...
          return res.status(200).json({
            success: false,
            error: '画像の生成に失敗しました。別のプロンプトで試してみてください。',
          });
        }
        ...
        if (isRateLimitError(errorMessage) && attempt < RETRY_DELAYS_MS.length) {
        ...
        if (isRateLimitError(errorMessage)) {
          return res.status(429).json({
            error: 'ただいまアクセスが集中しています。30秒ほど待ってから再度お試しください。',
          });
        }
